...
namespace Study.HR.Core.Application.Commands.EmployeeCommands
{
    ...
    public class UpdateEmploymentCommandHandler : CommandHandler<UpdateEmployeeCommand>
    {
        ...
        public UpdateEmploymentCommandHandler(IEmployeeRepository repository,
        ...
        public override async Task Handle(UpdateEmployeeCommand command, CancellationToken cancellationToken)
        {
            Employee employee = await _repository.FindOrThrowAsync(command.Id);
            await employee.ChangeCodeAsync(command.Code, _service);
